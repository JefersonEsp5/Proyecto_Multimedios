      ...
      <div v-else-if="store.tvShow" class="card">
         ...
         <div class="info">
            ...
            <div class="rating-row">
               ...
               <span class="details">• {{ store.tvShow.year || 'Desconocido' }} • {{ store.tvShow.runtime || 'No disponible' }} minutos</span>
            </div>
            <div class="genres">
                  ...
                  <template #item="{ item: trailer }">
                     <div class="trailer-card" @click="store.openTrailer(trailer.url)">
                        <div class="trailer-thumbnail">
                           <img :src="`https://img.youtube.com/vi/${getYouTubeID(trailer.url)}/mqdefault.jpg`"
               ...
               <GenericCarousel :items="store.filteredArtworks" :item-width="160" :scroll-amount="160">
                  <template #item="{ item: artwork, index }">
                     <div class="image-card" @click="store.openImage(artwork.image)">
                        <img :src="artwork.thumbnail || artwork.image" :alt="`Imagen ${index + 1}`"
                           class="image-thumb" />
                     </div>
                  </template>
               </GenericCarousel>
            ...
            <div class="episode-info-section">
               ...
               <div v-else class="next-episode-card">
                  <div class="episode-icon">❓</div>
                  <div class="episode-details">
                     <div class="episode-title">Información del próximo episodio no disponible.</div>
                  </div>
               </div>
            </div>
            ...
            <div class="seasons-section" v-if="store.tvShow.seasons && store.tvShow.seasons.length">
               ...
               <div v-for="season in store.sortedAndFilteredSeasons" :key="season.id" class="season-card-container">
                  <div class="season-card">
                     ...
                     <div class="season-info" @click="store.toggleSeasonExpanded(season.id)">
                  ...
                  <div v-if="season.expanded" class="episodes-list">
                     ...
                     <div v-if="!season.episodes || season.episodes.length === 0" class="no-episodes">
                        Cargando episodios o no hay episodios disponibles para esta temporada.
                     </div>
                  </div>
               </div>
            </div>
         ...
         <div v-if="store.activeTrailerUrl" class="modal-trailer" @click.self="store.closeTrailer">
            <div class="modal-content">
               <iframe width="100%" height="400"
                  :src="`https://www.youtube.com/embed/${getYouTubeID(store.activeTrailerUrl)}?autoplay=1`"
...
onMounted(() => {
   if (route.params.id && !store.tvShow) { // Solo busca si no hay datos cargados para evitar doble llamada en la carga inicial
